...
test(
  'preserves all 70 public routes, middleware configurations, and UUID matchers',
  { timeout: 15_000 },
  async () => {
    const audit = await registeredRoutes()

    assert.equal(contractsFixture.routes.length, 70)
    assert.equal(audit.routes.length, 70)
