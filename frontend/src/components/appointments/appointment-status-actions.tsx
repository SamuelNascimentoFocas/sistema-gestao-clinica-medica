...
> = {
  ...
  "no-show":
    "Confirma o registro de falta do paciente?",
};

const ACTION_SUCCESS_MESSAGE: Record<
  StatusAction | "cancel",
  string
> = {
...
export function AppointmentStatusActions({
  ...
  const [pendingAction, setPendingAction] =
    useState<StatusAction | "cancel" | null>(
      null,
    );

  const [
    isCancellationOpen,
    setIsCancellationOpen,
  ] = useState(false);

  const [
    cancellationReasonCode,
    setCancellationReasonCode,
  ] = useState<ManualCancellationReason>(
    "patient_request",
  );
  ...
  const canRegisterNoShow =
    renderedAt >= startsAt + 15 * 60 * 1000;
    ...
    <div className="mt-4 space-y-4 border-t pt-4">
      ...
      !isCancellationOpen ? (
        <p className="text-sm text-muted-foreground">
          As opções de conclusão e falta serão
          disponibilizadas após o horário do
          atendimento.
        </p>
      ) : null}
      ...
      {isCancellationOpen ? (
        ...
        >
          ...
          <div className="space-y-2">
            <Label
              htmlFor={`cancellation-reason-${appointment.id}`}
            >
              Motivo
            </Label>
            ...
            <select
              ...
              onChange={(event) =>
                setCancellationReasonCode(
                  event.target
                    .value as ManualCancellationReason,
                )
              }
            >
              {CANCELLATION_REASON_OPTIONS.map(
                (option) => (
                  <option
                    key={option.value}
                    value={option.value}
          ...
          <div className="space-y-2">
            <Label
              htmlFor={`cancellation-note-${appointment.id}`}
            >
              Observação
            </Label>
